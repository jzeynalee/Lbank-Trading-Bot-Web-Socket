...
PAIRS=['btc_usdt', 'eth_usdt', 'xrp_usdt', 'bnb_usdt', 'sol_usdt', 'doge_usdt',
    ...
    if len(symbol_data[pair][interval]) >= 30:
        ...
        for tf_label, tf in timeframes.items():
            if len(symbol_data[pair][tf]) >= 30:
                ...
                df_tf = (IndicatorCalculator(df_tf)
                         .calculate_rsi()
                         .calculate_ichimoku()
        ...
        if len(multi_df) == 5:
            orderbook_mock = {'bids': [{'quantity': 200}], 'asks': [{'quantity': 200}]}
            strat = StrategyEngine(multi_df)
            signal = (strat.analyze_trend()
            .analyze_signal()
            .analyze_entry()
            .analyze_scalping()
            .confirm_orderbook(orderbook_mock)
            .generate_signal())
...
async def run():
    asyncio.create_task(heartbeat())  # Background heartbeat task
